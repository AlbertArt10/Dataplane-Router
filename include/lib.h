...
// Definește dimensiunile headerelor pentru diferitele protocoale (Ethernet, IP, ICMP),
// precum și offseturile pentru calcularea pozițiilor specifice în cadrul pachetelor.
#define ETHER_HDR_SIZE sizeof(struct ether_header)    // Dimensiunea header-ului Ethernet
#define IP_HDR_SIZE sizeof(struct iphdr)              // Dimensiunea header-ului IP
#define ARP_HDR_SIZE sizeof(struct arp_header)
#define ICMP_HDR_SIZE sizeof(struct icmphdr)          // Dimensiunea header-ului ICMP
#define OFFSET_ICMP (ETHER_HDR_SIZE + IP_HDR_SIZE)      // Offset pentru începutul header-ului ICMP în pachet
#define OFFSET_ICMP_DATA (OFFSET_ICMP + ICMP_HDR_SIZE)  // Offset pentru datele ICMP din pachet
#define ICMP_PACKET_DATA_SIZE (IP_HDR_SIZE + 8)         // Dimensiunea datelor pachetului ICMP
